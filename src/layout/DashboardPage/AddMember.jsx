...
const AddMember = () => {
        ...
        if (selectedFile) {
            ...
            try {
                axios.post(
                    ...
                    navigate("/members/" + data.data.memberId, {state: {data: data.data}})
                }).catch(() => {
    ...
    useEffect(() => {
        if (Object.keys(validateMsg).length === 0) {
            ...
            axios.post("http://127.0.0.1:3000/API/members.php", formData)
                .then((data) => {
    ...
    const handleFileChange = (event) => {
        ...
        setFormData({...formData, img: '/public/images/' + file.name})
    ...
    return <>
        <div className='row'>
            <div className="card px-5">
                ...
                <form className='card-body pt-2 px-5 d-flex flex-column justify-content-center'
                    ...
                    <div className='px-5'>
                        <label htmlFor="name" className="control-label pt-0 pb-2 col-form-label">
                            Full Name <span className='text-danger'>*</span></label>
                        <div className={`input-group border rounded-3 ${validateMsg.name ? ' is-invalid ' : ' mb-3'}`}>
                            <span className="input-group-text bg-transparent px-6 border-0" id="basic-addon1"><i
                                className="ti ti-user fs-6"></i></span>
                            <input type="text" name='name'
                                   id='name' placeholder="John Deo"
                                   value={formData.name}
                        ...
                        <label htmlFor="email" className="control-label pt-0 pb-2 col-form-label">
                            Email <span className='text-danger'>*</span>
                        </label>
                        <div className={`input-group border rounded-3 ${validateMsg.email ? ' is-invalid ' : ' mb-3'}`}>
                            <span className="input-group-text bg-transparent px-6 border-0" id="basic-addon1"><i
                                className="ti ti-mail fs-6"></i></span>
                            <input type="text" id='email'
                                   placeholder="JohnDeo@example.com" name='email'
                        ...
                        <div className="input-group border rounded-3">
                            ...
                            <input type="number" id='phone' name='phone' className="form-control border-0 ps-2"
                                   value={formData.phone}
                                   onChange={(e) => setFormData({...formData, phone: e.target.value})}
                                   placeholder="John Deo"/>
                        </div>
                    </div>
                    <button type='submit' className="btn align-self-center w-25 mt-4 mb-5 btn-primary">Save</button>
                </form>
            </div>
        </div>
    </>
